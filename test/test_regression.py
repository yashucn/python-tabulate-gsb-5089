...
def test_sequence_arguments_accept_tuples():
    "Regression: sequence-type arguments accept tuples like equivalent lists"
    table = [["x" * 20, "y" * 20], ["1", "2"]]
    cases = [
        ({"rowalign": ("top",)}, {"rowalign": ["top"]}),
        ({"rowalign": ("top", "bottom")}, {"rowalign": ["top", "bottom"]}),
        ({"maxheadercolwidths": (3,)}, {"maxheadercolwidths": [3]}),
        ({"maxheadercolwidths": (3, 3)}, {"maxheadercolwidths": [3, 3]}),
        ({"maxcolwidths": (5,)}, {"maxcolwidths": [5]}),
        ({"maxcolwidths": (5, 5)}, {"maxcolwidths": [5, 5]}),
        ({"colalign": ("left",)}, {"colalign": ["left"]}),
        ({"colalign": ("left", "right")}, {"colalign": ["left", "right"]}),
        ({"headers": ("a", "b")}, {"headers": ["a", "b"]}),
    ]
    for tuple_kwargs, list_kwargs in cases:
        base = {"headers": ["aa", "bb"], "tablefmt": "grid"}
        tuple_result = tabulate(table, **{**base, **tuple_kwargs})
        list_result = tabulate(table, **{**base, **list_kwargs})
        assert_equal(list_result, tuple_result)


def test_sequence_argument_scalar_semantics_unchanged():
    "Regression: scalar rowalign/maxcolwidths behave as before the tuple fix"
    assert_equal(
        tabulate([[1, 2], [3, 4]], rowalign="top"),
        tabulate([[1, 2], [3, 4]], rowalign=["top", "top"]),
    )
    assert_equal(
        tabulate([["x" * 30]], maxcolwidths=5),
        tabulate([["x" * 30]], maxcolwidths=[5]),
    )


def test_maxcolwidths_respects_disable_numparse():
    "Regression: wrapping path must not numparse cells when numparse is disabled"
    for value in ["80,443", "100,120", "1,000"]:
        data = [["ports", value]]
        wrapped = tabulate(data, tablefmt="grid", disable_numparse=True, maxcolwidths=40)
        unwrapped = tabulate(data, tablefmt="grid", disable_numparse=True)
        assert_equal(unwrapped, wrapped)


def test_maxcolwidths_unparseable_number_strings_do_not_raise():
    "Regression: comma-separated strings must not raise with numparse enabled"
    for value in ["80,443", "100,120", "1,000"]:
        data = [["ports", value]]
        wrapped = tabulate(data, tablefmt="grid", maxcolwidths=40)
        unwrapped = tabulate(data, tablefmt="grid")
        assert_equal(unwrapped, wrapped)


def test_maxcolwidths_disable_numparse_preserves_strings():
    "Regression: number-like strings stay as-is when numparse is disabled"
    for value in ["007", "0012", "1_000", "1.50", "True", "0x10"]:
        result = tabulate([[value]], tablefmt="plain", disable_numparse=True, maxcolwidths=40)
        assert_equal(value, result)


def test_maxcolwidths_preserves_blank_lines():
    "Regression: blank lines inside cells survive wrapping"
    for cell in ["para 1\n\n para 2", "a\n\n\nb"]:
        wrapped = tabulate([[cell]], headers=["c"], tablefmt="grid", maxcolwidths=20)
        unwrapped = tabulate([[cell]], headers=["c"], tablefmt="grid")
        assert_equal(unwrapped, wrapped)


def test_maxcolwidths_blank_lines_still_wrap_text():
    "Regression: blank lines are preserved while long lines are still wrapped"
    expected = "\n".join(
        [
            "+------+",
            "| aaaa |",
    ...
    result = tabulate([["aaaa bbbb\n\ncccc"]], tablefmt="grid", maxcolwidths=5)
    assert_equal(expected, result)


def test_maxcolwidths_osc8_hyperlink_not_broken():
    "Regression: OSC 8 hyperlinks wrap by visible width and are never chopped"
    link = "\x1b]8;;http://example.com\x1b\\click here\x1b]8;;\x1b\\"
    result = tabulate([[link, "b"]], headers=["h1", "h2"], tablefmt="grid", maxcolwidths=[10, None])
    # visible text is exactly 10 columns wide: no wrapping may occur
    assert "http://example.com" in result
    assert "\x1b]8;;http://example.com\x1b\\click here\x1b]8;;\x1b\\" in result
    assert result.count("\x1b]8;;") == 2  # one opening, one closing sequence


def test_maxcolwidths_osc8_hyperlink_visible_width():
    "Regression: hyperlink cells are measured by their visible text width"
    link = "\x1b]8;;http://example.com\x1b\\click here\x1b]8;;\x1b\\"
    result = tabulate([[link]], tablefmt="plain", maxcolwidths=[10])
    assert_equal(link, result)


def test_maxcolwidths_csi_colors_still_wrap_by_visible_width():
    "Regression: CSI color sequences keep wrapping by visible width with resets"
    red = "\x1b[31m" + "word " * 6 + "\x1b[0m"
    result = tabulate([[red]], tablefmt="grid", maxcolwidths=11)
    assert "\x1b[31m" in result
    assert "\x1b[0m" in result
    # every wrapped line carries the color and is terminated by a reset
